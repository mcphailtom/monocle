...
- **MCP tools mode** runs a built-in MCP server (`monocle serve-mcp`) that exposes review operations as tools. This is the default for Claude Code and Pi. Pi reaches the server through `pi-mcp-adapter`.
...
```bash Pi
# Default: MCP tools mode through pi-mcp-adapter
monocle register pi

# To use skills/prompts mode instead:
