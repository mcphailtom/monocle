...
<Note>
  When the agent's working directory differs from monocle's working directory, use `--workdir` (or the `MONOCLE_WORKDIR` environment variable) to point commands at the correct repository. Alternatively, set `MONOCLE_SOCKET` to specify the socket path directly. Without one of these, commands derive the socket from the current directory and may not find a running session. Press `I` in the monocle TUI to see the active socket path.
</Note>
