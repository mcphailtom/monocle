...
<Tabs>
  ...
  <Tab title="Pi">
    ...
    By default, this uses MCP mode only if `pi-mcp-adapter` is already configured in Pi. In that case, monocle registers itself in `.pi/mcp.json` and installs prompt templates in `.pi/prompts/`; Pi calls monocle through the adapter's MCP gateway or direct `monocle_*` tools when available. If the adapter is not already configured, monocle falls back to skills/prompts and does not add the adapter package.
    ...
    Skills mode installs `SKILL.md` files in `.pi/skills/` plus the same prompt templates in `.pi/prompts/`. To add `pi-mcp-adapter` and use MCP tools explicitly, pass `--integration-mode mcp`. Pi still uses pull-based feedback; Claude MCP channels are not involved.
  </Tab>
