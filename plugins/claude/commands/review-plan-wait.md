...
3. **Run `monocle review send-artifact --wait`** using the Bash tool with:
   ...
   - `--type`: `md`
   - `--wait`: This flag blocks until the reviewer responds

4. **Handle the response:**
   - If the reviewer approved with no comments, inform the user and continue
   - If the reviewer provided feedback requesting changes, share the feedback with the user and act on it — update the plan, then run `monocle review send-artifact --wait` again to start another review round
